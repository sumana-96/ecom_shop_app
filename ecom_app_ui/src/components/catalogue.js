...
import { fetchCatelogues } from "../redux/catelogue/catelogueAction";
...
function Catalogue({ productData, fetchCatelogues }) {
  ...
  return productData && productData.all ? (
    <div>
      <h2>All Product List</h2>

      {productData &&
        productData.all &&
        productData.all.map((product) => (
          <table>
            <tr>
              <th>Name</th>
              <th>Description</th>
    ...
    <div>
      <div>
        {productData &&
          productData.all &&
          productData.all.map((product) => <p> {product.name}</p>)}
      </div>
    </div>
  );
  // <div>
...
const mapStateToProps = (state) => {
  return {
    productData: state.catelogueReducer,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    fetchCatelogues: () => dispatch(fetchCatelogues()),
  };
};
